...
interface Question {
  ...
  question: string;
  options: Option[];
  correctOption?: number; // Optional index of correct answer (0-3) if sent from server
}
...
export default function Index() {
  ...
  // Real-time calculation using (+4 for Correct, -1 for Wrong, 0 for Unattempted)
  ...
  const shareOnWhatsApp = () => {
    if (!result) return;
    const message = `🎯 NEET MCQ Practice Test Result:\nI scored ${result.score} marks out of ${result.maxScore || questions.length * 4}! 💪`;
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.card}>
          ...
          <View style={styles.markingRulesBox}>
            <Text style={styles.markingRuleText}> Marking Scheme applied:</Text>
            <Text style={styles.ruleDetail}>• Correct Answer: +4 Marks</Text>
            <Text style={styles.ruleDetail}>• Wrong Answer: -1 Mark</Text>
            <Text style={styles.ruleDetail}>• Unattempted: 0 Marks</Text>
          </View>
              ...
              return (
                ...
                >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={(e) => {
                      e.stopPropagation(); // Prevents selecting option when clicking audio icon
...
const styles = StyleSheet.create({
  ...
  markingRuleText: { color: '#F4F4F5', fontWeight: 'bold', marginBottom: 6, fontSize: 14 },
  ruleDetail: { color: '#A1A1AA', fontSize: 13, marginBottom: 2 },
  ...
  scoreText: { fontSize: 22, textAlign: 'center', marginBottom: 16, color: '#FF4D4D', fontWeight: 'bold' }
});